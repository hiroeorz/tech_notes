...
PR作成完了（初回レビューは自動で付く）
  ↓
デプロイ手順をユーザーへ提示（WSL実行用。未提示の場合のみ）
  ↓
...
コメント到着 → 各指摘の妥当性を検証
  ↓
妥当な指摘あり → 修正 → コミット(fix:) → プッシュ → 最新SHAでデプロイ手順を再提示 → 再レビュー依頼 ↺
  ↓
妥当な指摘なし → mainへマージ → 後片付け → 完了報告＋デプロイ手順を提示（マージコミットの完全SHA） → 終了
```
...
PR作成直後でまだデプロイ手順を提示していない場合（codex-review を単独で使用した場合など）は、`.opencode/skills/deploy-request/SKILL.md` に従い、WSLで実行するデプロイコマンド（`kamal deploy`）をユーザーへ提示してから待機ループに入る（エージェントは実行しない）。
...
後片付け完了後、**マージコミットの完全SHAを使ったデプロイ手順（`git pull` + `kamal deploy`）をユーザーへ提示**する（エージェントは実行しない。詳細は `.opencode/skills/deploy-request/SKILL.md`）。
