...
3. **環境変数・シークレット管理**
   ...
   * 必須シークレット: `RAILS_MASTER_KEY`, `POSTGRES_PASSWORD`, `ADMIN_PASSWORD`。
   * 初期管理者設定: `ADMIN_EMAIL`, `ADMIN_PASSWORD`。
   ...
   * Workers AI 利用時の必須シークレット: `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_AI_API_TOKEN`, `CLOUDFLARE_AI_MODEL`, `CLOUDFLARE_AI_TIMEOUT_SECONDS`, `CLOUDFLARE_AI_MAX_TOKENS`。
* Resend メール配信利用時の必須シークレット: `RESEND_API_KEY`（環境変数または `.kamal/secrets`）。
4. **DNS Rebinding 保護**
   * `config.hosts` に `APP_HOST` 環境変数の値のみを許可し、`/up` はヘルスチェック用に除外している。

---

## 5. デプロイ設定 (`config/deploy.yml`)

シングルVM環境における基本構成を以下に示す。各値は環境変数経由で注入する。
環境変数 `IMAGE`, `SERVER_IP`, `APP_HOST`, `REGISTRY_USERNAME`, `SSH_USER`, `POSTGRES_USER` を事前に設定すること。
...
* **コマンド例**:
  ...
  for db in _cache _queue _cable; do
    docker exec tech_notes-db pg_dump -U "$POSTGRES_USER" -Fc -d "tech_notes_production$db" \
...
2. `config/master.key` と `.kamal/secrets` に `POSTGRES_PASSWORD`, `RAILS_MASTER_KEY`, `ADMIN_EMAIL`, `ADMIN_PASSWORD` を安全な方法で設定。
3. 必要な環境変数（`IMAGE`, `SERVER_IP`, `APP_HOST`, `REGISTRY_USERNAME`, `SSH_USER`, `POSTGRES_USER`）をデプロイ環境に設定。
4. Cloudflare R2 のバケットを作成し、`rclone` 等の認証情報を VM に設定。
5. `kamal setup` を実行（プロキシの起動・DB accessory 起動・`db:prepare`・初回デプロイを一括実施）。
6. DNS で `APP_HOST` の値を VPS のパブリックIPに向ける（Let's Encrypt の発行に必要）。
7. `https://<APP_HOST>/` にアクセスして表示確認。
