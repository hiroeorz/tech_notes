...
環境変数 `IMAGE`, `SERVER_IP`, `PROXY_HOST`, `REGISTRY_USERNAME`, `SSH_USER`, `POSTGRES_USER` を事前に設定すること。
...
3. 必要な環境変数（`IMAGE`, `SERVER_IP`, `PROXY_HOST`, `REGISTRY_USERNAME`, `SSH_USER`, `POSTGRES_USER`, `APP_HOST`）をデプロイ環境に設定。
