...
- dump のアップロード先は Cloudflare R2 の `hiroe-tech-notes-backup` バケットとし、一時ファイルへ dump した後に `rclone copyto` で保存する。
...
```bash
docker exec "$CONTAINER" pg_dump -U hiroe -Fc -d "$DB"
```
...
docker exec "$CONTAINER" pg_dump -U hiroe -Fc -d "$DB" > "$DUMP_FILE"
rclone copyto "$DUMP_FILE" "r2:hiroe-tech-notes-backup/postgresql/${DB}_${STAMP}.dump" --s3-no-check-bucket
```
...
```text
/home/hiroe/ops/backup_postgres_to_r2.sh
```
...
- `hiroe` ユーザーで `rclone config` を実行し、`r2` remote を作成する。
- cron も `hiroe` ユーザーの crontab に登録する。
...
root の crontab から実行する場合、`hiroe` ユーザーの rclone 設定が読まれない可能性があるため、`--config` で設定ファイルを明示する。

```bash
rclone --config /home/hiroe/.config/rclone/rclone.conf copyto ...
```

### 7.2 DB 認証

`pg_dump` は PostgreSQL accessory コンテナ内で実行する。PostgreSQL コンテナ内では `POSTGRES_USER=hiroe` で初期化されているため、通常は以下で実行できる。

```bash
pg_dump -U hiroe -Fc -d tech_notes_production
```
...
```text
r2:hiroe-tech-notes-backup/postgresql/
```
...
```bash
pg_restore -U hiroe -d tech_notes_production_restore backup.dump
```
