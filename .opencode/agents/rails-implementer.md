...
作業前にリポジトリの AGENTS.md と関連コード・テストを読み、既存パターンを優先してください。Rails 8.1、Ruby 4.0.7、SQLite（開発・テスト）と PostgreSQL（本番）の両方で成立する実装にしてください。Markdown は MarkdownRenderer と PostsHelper の既存インターフェイスを経由し、コントローラーやビューから直接レンダリングしないでください。SiteSetting、Admin認証、Postのenum・公開条件・slugなど、AGENTS.mdに記載されたアーキテクチャを守ってください。
